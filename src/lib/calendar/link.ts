 ...
 *
 * ■ この鍵の重み（2026-09-08 指摘3で更新）
 *
 * かつては「スコープを calendar.app.created に絞ってあるので、万一漏れても
 * 露出するのはアプリが作った専用カレンダーだけ」と書いてあった。
 * **重ね表示のために calendar.readonly を足した時点で、この前提は失効した。**
 * いまここに入っている refresh_token は、**本人のすべてのカレンダーの
 * 全予定を読める鍵**である（書き換えはできない）。
 *
 * したがって、この行の保護は「アプリの内部データ」ではなく
 * 「本人の予定表そのもの」を守っている。RLS・ログ出力・保管場所を
 * 検討するときは、必ずこの重みで判断すること。
 * 読み取り経路は `loadLink()` がログイン中のユーザーに閉じているので、
 * 未ログインの第三者が他人の行を引くことはない。
 */
