...
export interface HabitStats {
  /**
   * 数える単位。週N回だけ "week"（曜日を決めない約束なので、日で数えると
   * きっちり守っていても達成率が n/7 に潰れる）。ほかは "day"。
   * streak と planned の単位がこれで変わるので、画面では必ず出しわける。
   */
  unit: "day" | "week";
  /** 直近の達成率（0〜1）。窓は日なら30日、週なら終わった4週 */
  rate: number;
  /** 集計に使った分母。日なら予定日数、週なら回数。少ないうちは率を出しても意味がない */
  planned: number;
  /** 連続。日なら予定日、週なら目安に届いた週の数。partial は途切れとしない */
  streak: number;
